@php
    ...
    $pageTitle = $product->name . ' - ' . ($product->brand ? $product->brand->name . ' ' : '') . 'Buy Online';
    $pageDescription = $product->short_description ?: 'Buy ' . $product->name . ' online at the best price. ' . ($product->is_sale ? 'On sale now!' : 'Free shipping on eligible orders.');
    ...
    $wishlistClass = 'btn-outline-dark-3';
    $compareClass = 'btn-outline-dark-3';
@endphp
        ...
        <div class="container">
                        ...
                        <div class="product-details">
                                    ...
                                    @foreach($grouped as $attributeName => $attributeVariants)
                                        <div class="product-variant-group mb-3">
                                            ...
                                            <div class="variant-options">
                                                @foreach($attributeVariants as $index => $variant)
                                                    <button type="button"
                                                        ...
                                                        data-image="{{ str_starts_with($product->image, 'assets/') ? asset($product->image) : asset('storage/' . $product->image) }}"
                            ...
                            <div class="product-details-footer">
                                <div class="product-cat">
                                    @if($product->category)
                                        Category: <a href="{{ url('/') }}">{{ $product->category->name }}</a>
                                    @endif
                                </div>
            ...
            <div class="product-details-tab mt-5">
                <ul class="nav nav-pills" id="product-tab" role="tablist">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" id="shipping-tab" data-bs-toggle="pill" href="#shipping" role="tab" aria-controls="shipping" aria-selected="false">Shipping & Returns</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" id="reviews-tab" data-bs-toggle="pill" href="#reviews" role="tab" aria-controls="reviews" aria-selected="false">
                ...
                <div class="tab-content" id="product-tab-content">
                    ...
                    <div class="tab-pane fade" id="specifications" role="tabpanel" aria-labelledby="specifications-tab">
                        <div class="product-specification">
                            <table class="table table-bordered table-striped specification-table">
                                <tbody>
                        ...
                        @foreach($relatedProducts as $related)
                            @php
                                $relatedImage = $related->image
                                    ? (str_starts_with($related->image, 'assets/') ? asset($related->image) : asset('storage/' . $related->image))
                                    : asset('assets/images/products/placeholder.jpg');
                            @endphp
...
(function($) {
    $(function() {
        ...
        function handleAction(e) {
            ...
            var action = $btn.attr('id') || 'unknown';
