...
@section('header-title', 'Add Trending Banner')

@section('content')
<h2>Add Trending Section Banner</h2>

<form action="{{ route('admin.trending-products.banner.store') }}" method="POST" enctype="multipart/form-data">
    @csrf
    ...
    <div class="card mb-3">
        ...
        <div class="card-body">
            <div class="mb-3">
                ...
                <input type="text" name="title" id="title" class="form-control @error('title') is-invalid @enderror" value="{{ old('title') }}" placeholder="e.g., Trending Products">
                @error('title')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>

            <div class="mb-3">
                <label for="subtitle" class="form-label">Subtitle / Offer Text</label>
                <input type="text" name="subtitle" id="subtitle" class="form-control @error('subtitle') is-invalid @enderror" value="{{ old('subtitle') }}" placeholder="e.g., Up to 50% off">
                @error('subtitle')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>

            <div class="mb-3">
                <label for="highlight_text" class="form-label">Highlight Text</label>
                <input type="text" name="highlight_text" id="highlight_text" class="form-control @error('highlight_text') is-invalid @enderror" value="{{ old('highlight_text') }}" placeholder="e.g., Limited time offer">
            ...
            <div class="mb-3">
                ...
                <select name="product_id" id="product_id" class="form-control @error('product_id') is-invalid @enderror" required>
                    ...
                    @foreach($products as $product)
                        <option value="{{ $product->id }}" {{ old('product_id') == $product->id ? 'selected' : '' }}>{{ $product->name }} - ${{ number_format((float) $product->price, 2) }}</option>
                    @endforeach
                </select>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="button_text" id="button_text" class="form-control @error('button_text') is-invalid @enderror" value="{{ old('button_text', 'Shop Now') }}" placeholder="Shop Now">
    ...
    <div class="card mb-3">
        ...
        <div class="card-body">
            <div class="mb-3">
                ...
                <small class="text-muted">Recommended: 600 x 800 px. Max 4MB.</small>
            </div>

            <div class="row">
                <div class="col-md-6 mb-3">
                    <label for="background_color" class="form-label">Background Color</label>
                    <input type="color" name="background_color" id="background_color" class="form-control @error('background_color') is-invalid @enderror" value="{{ old('background_color', '#f5f5f5') }}">
                    @error('background_color')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-6 mb-3">
                    <label for="text_color" class="form-label">Text Color</label>
                    <input type="color" name="text_color" id="text_color" class="form-control @error('text_color') is-invalid @enderror" value="{{ old('text_color', '#333333') }}">
