    ...
    <div class="card mb-3">
        ...
        <div class="card-body">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="slug" id="slug" class="form-control @error('slug') is-invalid @enderror" value="{{ old('slug', $product->slug) }}" placeholder="e.g., apple-ipad-pro">
    ...
    <div class="card mb-3">
        ...
        <div class="card-body">
            <div class="mb-3">
                ...
                @if($product->image)
                    @php
                        $mainImage = str_starts_with($product->image, 'assets/') ? asset($product->image) : asset('storage/' . $product->image);
                    @endphp
            ...
            <div class="mb-3">
                ...
                @endif
                <input type="file" id="gallery-images" name="gallery[]" id="gallery" class="form-control @error('gallery') is-invalid @enderror" accept="image/*" multiple>
    ...
    <div class="card mb-3">
        ...
        <div class="card-body">
            <div class="mb-3">
                ...
                <textarea name="short_description" id="short_description" class="form-control @error('short_description') is-invalid @enderror" rows="2" placeholder="Brief product summary shown on cards...">{{ old('short_description', $product->short_description) }}</textarea>
            ...
            <div id="specifications-container">
                ...
                @forelse($product->specifications as $spec)
                    <div class="row g-2 specification-row mb-2">
                        <div class="col-md-5">
                            <input type="text" name="specifications[{{ $specIndex }}][key]" class="form-control" value="{{ old('specifications.' . $specIndex . '.key', $spec->key) }}" placeholder="Specification name">
                        </div>
                        <div class="col-md-5">
                            <input type="text" name="specifications[{{ $specIndex }}][value]" class="form-control" value="{{ old('specifications.' . $specIndex . '.value', $spec->value) }}" placeholder="Specification value">
                        </div>
                        <div class="col-md-2">
                ...
                @empty
                    <div class="row g-2 specification-row mb-2">
                        <div class="col-md-5">
                            <input type="text" name="specifications[0][key]" class="form-control" placeholder="Specification name">
                        </div>
                        <div class="col-md-5">
    ...
    <div class="card mb-3">
        ...
        <div class="card-body">
            <div class="mb-3">
                ...
                <textarea name="shipping_information" id="shipping_information" class="form-control @error('shipping_information') is-invalid @enderror" rows="3" placeholder="Shipping details, delivery time, costs...">{{ old('shipping_information', $product->shipping_information) }}</textarea>
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="number" name="order" id="order" class="form-control @error('order') is-invalid @enderror" value="{{ old('order', $product->order) }}">
...
<div class="card mb-3" id="twinvexa-gallery-dashboard">
    ...
    <div class="card-body">
        @php
            ...
            if (isset($product)) {
                $existingGalleryImages = \App\Models\ProductImage::where(
                    'product_id',
                    $product->id
                ...
                @foreach($existingGalleryImages as $galleryImage)
                    <div class="col-md-3 col-sm-4 col-6">
                        <div class="card h-100">
                            <img
                                src="{{ \Illuminate\Support\Str::startsWith($galleryImage->image, ['http://', 'https://'])
            ? $galleryImage->image
            : asset(ltrim($galleryImage->image, '/')) }}"
                                class="card-img-top"
        ...
        <input
            type="file"
            id="gallery-images" name="gallery[]"
            class="form-control"
            accept="image/jpeg,image/png,image/webp"
            multiple>

        <small class="text-muted">
            You can select multiple product images. Existing images will remain unless deleted.
        </small>
    </div>
</div>
...
@push('scripts')
<script id="twinvexa-product-editor"
        src="https://cdn.jsdelivr.net/npm/tinymce@7/tinymce.min.js"></script>

<script>
document.addEventListener('DOMContentLoaded', function () {
    if (window.tinymce && document.querySelector('#description')) {
        tinymce.init({
    ...
    function reindexRows() {
        container.querySelectorAll('.specification-row').forEach(function (row, index) {
    ...
    addButton.addEventListener('click', function () {
        ...
        row.className = 'row g-2 specification-row mb-2';
    ...
    container.addEventListener('click', function (event) {
        if (event.target.classList.contains('remove-specification')) {
            ...
            if (rows.length > 1) {
                ...
                event.target.closest('.specification-row')
...
@push('scripts')
<script id="twinvexa-gallery-multiple-upload-fix">
document.addEventListener('DOMContentLoaded', function () {
    ...
    input.addEventListener('change', function () {
        Array.from(input.files).forEach(function (file) {
            const exists = Array.from(selectedFiles.files).some(function (oldFile) {
    ...
    function renderPreview() {
        ...
        Array.from(selectedFiles.files).forEach(function (file, index) {
            ...
            reader.onload = function (event) {
                ...
                col.innerHTML = `
                    <div class="card h-100">
                        ...
                        <div class="card-body p-2 text-center">
                            ...
                            <button type="button"
                                    class="btn btn-danger btn-sm remove-new-gallery-image">
                                Remove
                            </button>
                        </div>
                    </div>
                ...
                col.querySelector('.remove-new-gallery-image')
                    .addEventListener('click', function () {
                        const newFiles = new DataTransfer();

                        Array.from(selectedFiles.files).forEach(function (item, itemIndex) {
                            if (itemIndex !== index) {
                                newFiles.items.add(item);
                            }
                        });
