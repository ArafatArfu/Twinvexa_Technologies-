...
<div class="product product-2">
    <figure class="product-media">
        @foreach($labels as $label)
            ...
            @endif
        @endforeach
        <a href="#"><img src="{{ asset($image) }}" alt="Product image" class="product-image"></a>
        @if($hoverImage)
    ...
    <div class="product-body">
        <div class="product-cat"><a href="#">{{ $category }}</a></div>
        ...
        @if(count($colors) > 0)
            <div class="product-nav product-nav-dots">
                @foreach($colors as $index => $color)
                    <a href="#" @if($index === 0) class="active" @endif style="background: {{ $color }};"><span class="sr-only">Color name</span></a>
                @endforeach
            </div>
        @endif
    </div>
